...
_*Inspired by*_ the Helm charts https://github.com/confluentinc/cp-helm-charts[Confluent Inc.'s cp-helm-charts] and https://github.com/rafaelmnatali/kafka-k8s[Rafael Natali's kafka-k8s] for deploying Confluent platform components and demonstrating Kafka deployment on Kubernetes in Zookeeper and KRaft modes.
