...
=== Mandatory
- *make*: For executing instructions in the Makefile. Ensure compatibility with your project by using version X.X or newer.
- *docker* and *docker-compose*: For running services in containers. Docker Desktop version 4.27.2 or newer is recommended. Note that Docker Desktop integrates `docker-compose`.
- *helm*: For deploying on Kubernetes. Version X.X or newer is recommended.
